...
- [x] Flask 웹 대시보드 (`dashboard.py`, 포트 5000)
  ...
  - [x] 최근 30건 거래 내역 테이블
  - [x] 30초 자동 새로고침
- [x] 텔레그램 `/snapshot` 명령 — 대시보드 이미지(PNG)를 텔레그램으로 전송 (`snapshot.py`, NanumGothic 한글 폰트)
...
- [x] KIS 주문 API 500 오류 — 모의투자 주문 포트 9443 → 29443 수정 (`KIS_ORDER_BASE_URL`)
- [x] KIS 잔고 API 500 오류 — kis_balance.py도 동일 포트 수정
- [x] positions 공유 버그 — `sys.modules['main'] = sys.modules['__main__']` 로 수정
- [x] `/balance` 잔여예수금 오류 — `realized_pnl` 누락 수정
- [x] `log_trade()` entry_time 누락 — cmd_sell / cmd_sellall 수정
- [x] `calc_quantity` max(1,...) → 예산 부족 시 0 반환으로 수정
- [x] 매수 예산 5% 안전마진 적용 — 시장가 슬리피지/수수료 대비
- [x] 손절 기준 변경 — STOP_LOSS_RATE 10% → 5%, EMERGENCY_STOP_RATE 15% → 8%
...
- [ ] 베이시스 필터 임계값 결정 — 데이터 충분히 쌓인 후 (백워데이션 < -0.3% 매수 보류 검토)
