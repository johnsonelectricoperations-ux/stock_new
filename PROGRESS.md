...
미해결 근본원인(사용자 결정 대기). 예수금 vs 주문가능금액 불일치 → 매수 스킵 발생(자본 미활용). 제대로 고치려면 KIS 매수가능조회(inquire-psbl-order, VTTC8908R) API로 실제 주문가능액 기준 사이징 필요. 실전(9443) 전환 시 더 중요 → real_trading_transition 체크리스트 후보. 라이브 API 검증 필요.
