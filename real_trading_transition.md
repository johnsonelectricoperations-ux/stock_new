...
## 9. 베이시스 필터 및 선물 데이터 수집 (실전 전환 후)

**모의투자 중 선물 데이터 수집 불가** — 두 가지 경로 모두 실패 확인 (2026-05-18).
...
현재 `basis_log.csv`에는 현물(KODEX 200 기반) 지수만 기록됨. 선물 컬럼은 공백.

**실전 전환 후 할 일.**
1. KIS 실전 서버에서 `FHKIF03010100` TR 동작 여부 확인
2. 동작 시 `basis_collector.py`의 `_get_kospi200_futures()`를 KIS API 방식으로 교체
3. 데이터 충분히 쌓인 후 베이시스 임계값 결정
