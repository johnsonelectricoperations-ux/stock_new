...
## 검증 결과 (2026-06-25, 거래 29건 — 1차 중간점검)

> 선행조건(30건)에 1건 미달이나 사용자 요청으로 29건에서 중간 점검. 피처 완비 표본은 N=11(bb_pct/atr/avg_tr가 채워진 최근 거래)뿐이고 kospi_trend가 전건 bullish(하락장 0건)라, **파라미터 변경은 과적합 위험으로 보류.** 아래는 현시점 신호와 재검토 시점.

### 전반 성과
- 승률 41.4%(12승17패), 손익비 2.21(평균익 +12.6%/평균손 −5.7%), 기대값 +1.87%/건, 누적 +1,044,680원. 양의 기대값 — 구조는 건강.
- 06-18 고점(+2.33M)→06-24(+1.04M) 1주간 −128만원 반납(7연속 손절). → 장중 급락일 동반손절이 핵심 실패. **대응: 장중 코스피 급락 가드 신설(−1.5%, .env MARKET_CRASH_GUARD_RATE). 파라미터 튜닝 아님.**

### 항목별 신호 (followup_log 포함)
| 항목 | 신호 | 판정 |
|---|---|---|
| 손절 −5% | 매도후 추적 10건 중 7건 "좋은손절"(계속 하락, d20 평균 −11%), 3건 휩쏘(에스에이엠티 d3 +33% 등) | **유지** — 느슨하게 풀면 7건 손실이 2~4배 |
| 모멘텀익절 +10% | 매도후 d3 +6.9%→d20 −7.5%. 일부 더 감(주성 +23, LG전자 d10 +81)·일부 급락회피(LS −34, 현대모비스 −23). 방향성 없는 wash | **유지** |
| 모멘텀 필터(하한 ≥15) | 고모멘텀(90~200) 구간이 가장 덜 나쁨 → 고모멘텀 추구 설계를 **지지**(반박 아님). #9 "모멘텀 상한 추가" 불필요 | **유지(상한 미추가)** |
| BB%B(하드컷 0.95/우선 0.85) | (0.92~1.0] 버킷이 오히려 +5.28% — 단 2건(미래에셋생명 중복)이라 노이즈 | **유지** — 단 0.85 페널티 과한지 열린 질문 |
| 트레일 12% | 트레일링스탑 발동 사례 거의 없음(승자 대부분 모멘텀익절로 청산) | **평가 불가** |
| 부분익절 +20% | N=1(레이저쎌) | **평가 불가** |

### 재검토 대기 (N 확대 후)
- **BB%B 0.85 페널티 완화 후보** — bb_pct 30건+ 축적 후 (0.85~0.95) 구간 성과 재검증. 유의미하게 나쁘지 않으면 BB_PCT_PREFER 상향 검토. (현재 근거 2건 = 노이즈, 완화는 리스크 증가 방향이라 보류)
- **손절 휩쏘율** — 30→50건에서 휩쏘(손절후 반등)가 계속 30%+면 "캔들확인 2회" 등 강화 검토. 현재는 보호 역할 우세라 유지.
- **급락 가드 임계값(−1.5%)** — market_log.csv 분당 궤적 2~3주 축적 후 '레벨 vs 기울기'(장초반 하락 기울기) 비교해 재보정.
